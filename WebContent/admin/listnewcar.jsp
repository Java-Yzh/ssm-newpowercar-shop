			...
			<div class="widget">
				...
				<table cellpadding="0" cellspacing="0" width="100%" class="sTable">
					...
					<c:forEach items="${newcarList}" var="newcar">
						<tr align="center">
							<td>${newcar.newcarname}</td>
							<td>${newcar.catename}</td>
							<td>${newcar.price}</td>
							<td>${newcar.recommend}</td>
							<td>${newcar.thestart}</td>
							<td>${newcar.theend}</td>
							<td>${newcar.hits}</td>
							<td>${newcar.sellnum}</td>
							<td><a href="newcar/getNewcarById.action?id=${newcar.newcarid}">编辑</a>&nbsp;&nbsp;<a
								href="newcar/deleteNewcar.action?id=${newcar.newcarid}" onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a></td>
						</tr>
					</c:forEach>
				</table>
