...
public class IndexAction extends BaseAction {
	...
	// 公共方法 提供公共查询数据
	private void front() {
		this.getRequest().setAttribute("title", "浩途新能源汽车网-力作中国最大的新能源汽车网站");
		List<Cate> cateList = this.cateService.getAllCate();
		this.getRequest().setAttribute("cateList", cateList);
		List<Newcar> hotList = this.newcarService.getNewcarByHot();
	...
	// 首页显示
		...
		public String index() {
			...
			for (Cate cate : cateList) {
				List<Newcar> flimList = this.newcarService.getNewcarByCate(cate.getCateid());
	...
	public String addcart() {
		...
		String userid = (String) this.getSession().getAttribute("userid");
		Cart cart = new Cart();
		cart.setNewcarid(getRequest().getParameter("goodsid"));
	...
	public String preCheckout() {
		...
		List<Cart> cartList = this.cartService.getCartByCond(cart);
		if (cartList.size() == 0) {
			this.getRequest().setAttribute("message", "请选购新能源汽车商品");
			return "redirect:/index/cart.action";
		}
	...
	public String checkout() {
		...
		if (cartList.size() == 0) {
			...
			for (Cart cart : cartList) {
				...
				details.setNewcarid(cart.getNewcarid());
				...
				Newcar goods = this.newcarService.getNewcarById(cart.getNewcarid());
				goods.setSellnum("" + (Integer.parseInt(goods.getSellnum()) + Integer.parseInt(cart.getNum())));
				this.newcarService.updateNewcar(goods);
	...
	public String cate(String id, String number) {
		this.front();
		Newcar goods = new Newcar();
		goods.setCateid(id);
		List<Newcar> flimList = new ArrayList<Newcar>();
		List<Newcar> tempList = this.newcarService.getNewcarByCond(goods);
		int pageNumber = tempList.size();
		int maxPage = pageNumber;
		if (maxPage % 12 == 0) {
			maxPage = maxPage / 12;
	...
	// 推荐产品
		...
		public String getHost(String number) {
			...
			goods.setRecommend("是");
			List<Newcar> flimList = new ArrayList<Newcar>();
			List<Newcar> tempList = this.newcarService.getNewcarByHot();
			int pageNumber = tempList.size();
			int maxPage = pageNumber;
			if (maxPage % 12 == 0) {
				maxPage = maxPage / 12;
	...
	public String all(String number) {
		this.front();
		List<Newcar> flimList = new ArrayList<Newcar>();
		List<Newcar> tempList = this.newcarService.getAllNewcar();
		int pageNumber = tempList.size();
		int maxPage = pageNumber;
		if (maxPage % 12 == 0) {
			maxPage = maxPage / 12;
	...
	public String query(String name) {
		this.front();
		Newcar goods = new Newcar();
		goods.setNewcarname(name);
		List<Newcar> flimList = this.newcarService.getNewcarByLike(goods);
		this.getRequest().setAttribute("flimList", flimList);
		return "users/list";
	}
	...
	public String detail(String id) {
		this.front();
		Newcar goods = this.newcarService.getNewcarById(id);
	...
	public String addTopic(Topic topic) {
		...
		topic.setNewcarid(this.getRequest().getParameter("goodsid"));
		topic.setNum(this.getRequest().getParameter("num"));
		topic.setUsersid(userid);
		this.topicService.insertTopic(topic);
		return "redirect:/index/detail.action?id=" + topic.getNewcarid();
	}
